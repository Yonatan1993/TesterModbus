...
public class ModbusHandler implements Cloneable {
    ...
    public void relayShort(byte relayNumber, int timeout, IResultListener resultListener) {
        short status = 0;
        byte[] relayOnMsg = buildSetRelayStateCommand(true, (byte) relayNumber);
        com.write(relayOnMsg, " relay " + (relayNumber + 1) + " ON ");
        messageBuffer = com.getQueue();
        try {
            List<Byte> poll = messageBuffer.poll(50, TimeUnit.MILLISECONDS);
            if (poll == null) {
                status = 1;
                resultListener.RelayOnResult(status, new TesterExceptions(ErrorCodeEnum.RELAY_CLOSE_CIRCUIT));
            } else {
                resultListener.RelayOnResult(status, null);
            }
        ...
        new Timer().schedule(new TimerTask() {
            ...
            public void run() {
                short status = 0;
                byte[] relayOffMsg = buildSetRelayStateCommand(false, (byte) relayNumber);
                com.write(relayOffMsg, " relay " + (relayNumber + 1) + " OFF ");
                messageBuffer = com.getQueue();                                         //TODO: ASK ASAF
                try {
                    List<Byte> poll = messageBuffer.poll(50, TimeUnit.MILLISECONDS);
                    if (poll == null) {
                        status = 1;
                        resultListener.RelayOffResult(status, new TesterExceptions(ErrorCodeEnum.RELAY_OPEN_CIRCUIT));
                    } else {
                        resultListener.RelayOffResult(status, null);
                    }
                } catch (InterruptedException e) {
    ...
    public void setRelayState(boolean power, byte relayNumber, IResultListener resultListener) throws TooManyListenersException, InterruptedException, TimeoutException {
    ...
    public int readInputs() throws InterruptedException, TesterExceptions {
        byte[] dataBytes = {0x01, 0x02, 0x00, 0x00, 0x00, 0x08};
        com.write(writeDataWithCRC(dataBytes), " Sending read input command: ");
        MessageParser.ModbusMessage modbusMessage = listenToModbusResponse(); //TODO: need to add an exception which deals with  read error
        int value = 0;
    ...
    private MessageParser.ModbusMessage listenToModbusResponse() throws InterruptedException, TesterExceptions {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (newBytes != null) {
                printInputResponseCommand(newBytes);
                status = messageParser.parseData(newBytes);//TODO: need to add an exception which deals with  parsing error
        ...
        throw new TesterExceptions(ErrorCodeEnum.MODBUS_NOT_LISTENING);
    }

    private void printInputResponseCommand(List<Byte> newBytes) {
        Byte[] rezBytes = newBytes.toArray(new Byte[newBytes.size()]);
        byte[] byets = new byte[rezBytes.length];
        for (int i = 0; i < rezBytes.length; i++)
            byets[i] = rezBytes[i];
        System.out.println("*******Input Relay state command " + Com.bytesToHexStr(byets));
    }
